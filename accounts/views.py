...
def add_to_cart(request, uid):
    try:
        ...
        color_variant = get_object_or_404(
            ColorVariant, color_name=color_name) if color_name else None
    ...
    if request.GET.get('buy_now') == '1' and settings.DEBUG:
        return redirect('demo_checkout')
...
def cart(request):
    ...
    if cart_obj:
        ...
        try:
            ...
            payment = None
            messages.warning(
                request, 'Payment gateway is not configured for local testing. Your cart is ready.')

    context = {
        'cart': cart_obj,
        'payment': payment,
        'auto_checkout': request.GET.get('buy_now') == '1',
...
def demo_checkout(request):
    ...
    cart.razorpay_order_id = f'demo_{uuid.uuid4().hex[:20]}'
    cart.is_paid = True
    cart.save(update_fields=['razorpay_order_id', 'is_paid'])
    order = create_order(cart)
